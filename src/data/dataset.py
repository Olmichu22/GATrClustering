...
class ClusteringSDHCALDataset(Dataset):
    ...
    def train_feature_values(self, train_idx: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        if self.event.get("energy") is not None:
            out["energy"] = np.asarray(self.event["energy"])[ev_global].astype(np.float64)
        return out
    ...
    def apply_scaling_inplace(self, scaler: FeatureScaler) -> None:
        ...
        if self.event.get("energy") is not None:
            self.event["energy"] = scaler.apply("energy", np.asarray(self.event["energy"]))
