...
Each feature has its own ``mode``:
    ...
    log_z  : (log1p(x) - mean) / std, with mean/std OF THE LOGGED values

``log_z`` exists for heavy-tailed positive quantities such as the per-event hit
density (median ~1.9 hits/layer but a 10% shower tail reaching ~46): plain
z_norm leaves that tail at +6 sigma, plain log leaves a non-unit scale. The
stats are computed online over the train split like every other feature, so a
different dataset re-derives them instead of carrying hardcoded constants.
    none   : unchanged
...
class FeatureScaler:
    ...
    def needs_stats(self) -> bool:
        """True if any feature uses z_norm/minmax/log_z (log/none need no stats)."""
        return any(m in ("z_norm", "minmax", "log_z") for m in self.modes.values())
    ...
    def resolve_stats(
        ...
        # log_z: the stats describe the LOGGED variable, so transform before
        # measuring -- otherwise mean/std would belong to the raw scale and the
        # subsequent (log1p(x) - mean)/std would be meaningless.
        feats_needed = {}
        for f, v in train_values_by_feature.items():
            mode = self.modes.get(f)
            if mode in ("z_norm", "minmax"):
                feats_needed[f] = v
            elif mode == "log_z":
                feats_needed[f] = np.log1p(np.asarray(v, dtype=np.float64))
    ...
    def apply(self, feat: str, arr: np.ndarray) -> np.ndarray:
        ...
        if mode == "log_z":
            return (np.log1p(arr) - s["mean"]) / s["std"]
        if mode == "minmax":
            rng = s["max"] - s["min"]
